...
public class DeployCommand extends AbstractSapHanaCloudCommand {
	...
	try {
	    String[] parameters = getHanaCloudClientParams(
		    getSelectedProject(context), STOP_CMD, sdkLocation,
		    account, userName, password);
	    runHanaCloudClient(getSelectedProject(context), parameters);

	    parameters = getHanaCloudClientParams(getSelectedProject(context),
		    DEPLOY_CMD, sdkLocation, account, userName, password);
	    runHanaCloudClient(getSelectedProject(context), parameters);

	    parameters = getHanaCloudClientParams(getSelectedProject(context),
		    START_CMD, sdkLocation, account, userName, password);
	    runHanaCloudClient(getSelectedProject(context), parameters);
	} catch (Exception e) {
	...
	return Results
		.success("Application was deployed successfully to the cloud");
    }

    private String runHanaCloudClient(Project project, String[] parameters)
	    throws Exception {
	String command = "java";
	if (OperatingSystemUtils.isWindows()) {
	    command += ".exe";
	}
	OsProcessExecutor.execute(project.getRootDirectory()
		.getUnderlyingResourceObject(), command, parameters);
	return command;
    }

    private String[] getHanaCloudClientParams(Project project, String command,
	    String sdkLocation, String account, String userName, String password) {
	String applicationName = project.getFacet(MetadataFacet.class)
		.getProjectName();
	String archiveLocation = project.getFacet(PackagingFacet.class)
		.getFinalArtifact().getFullyQualifiedName().replace("\\/", "/");
	if (OperatingSystemUtils.isWindows()) {
	    archiveLocation = archiveLocation.replace('/', '\\');
	}

	List<String> commonParameters = Arrays.asList("-cp",
		getClassPath(sdkLocation), CLIENT_MAIN_CLASS, command,
		"--host", DEFAULT_HOST, "--account", account, "--user",
		userName, "--application", applicationName, "--password",
		password);
